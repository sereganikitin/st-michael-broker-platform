...
async function main() {
  ...
  try {
    ...
    const stats = { matched: 0, toDirect: 0, alreadyDirect: 0, hasBroker: 0, notFound: 0, multi: 0, toBroker: 0 };
    const directIds = [];
    const notFound = [];
    ...
    for (let i = 0; i < brokerIds.length; i += 200) {
      ...
      const res = await prisma.registryDeal.updateMany({ where: { id: { in: batch }, brokerId: { not: null }, saleChannel: null }, data: { saleChannel: "BROKER" } });
