...
export class AmoCrmAdapter {
  ...
  async createBrokerLeadFromLanding(data: {
    ...
    try {
      ...
      if (lead?.id) {
        ...
        try {
          // 2026-06-30: формат чеклист-задачи. Менеджер видит чёткие пункты
          // что нужно сделать, в комментарии к задаче отписывает по факту.
          const checklistText = [
            `ЗАДАЧА: связаться с новым брокером и провести знакомство.`,
            ``,
            `ЧТО НУЖНО СДЕЛАТЬ:`,
            `1. Позвонить по номеру ${data.brokerPhone}`,
            `2. Подтвердить что это реальный человек, заинтересованный в сотрудничестве`,
            `3. Рассказать кратко: что такое St Michael, какие проекты (Зорге 9 и СБ), как устроена комиссия`,
            `4. Объяснить вход в кабинет: broker.stmichael.ru → «Забыли пароль» → ввод телефона → ссылка на email`,
            `5. Объяснить процесс фиксации клиента (30 дней уникальности)`,
            `6. Уточнить ИНН агентства / готовность работать как ИП`,
            ``,
            `ДАННЫЕ:`,
            `• Имя: ${data.brokerName}`,
            `• Телефон: ${data.brokerPhone}`,
            ...(data.brokerEmail ? [`• Email: ${data.brokerEmail}`] : []),
            `• Источник: ${data.source === 'LANDING_BROKER_TOUR' ? 'Запись на брокер-тур (лендинг)' : 'Форма «Связаться с нами» (лендинг)'}`,
            ``,
            `ПОСЛЕ ЗВОНКА:`,
            `— заполните комментарий к задаче (по факту что сделано)`,
            `— переведите лид в нужный статус воронки`,
          ].join('\n');
  ...
  async addRefixationAttemptNote(leadId: number, data: {
    ...
    try {
      ...
      // 2026-06-30: формат чеклист-задачи.
      const checklistText = [
        `⚠ ЗАДАЧА: разрешить конфликт повторной фиксации.`,
        ``,
        `СИТУАЦИЯ:`,
        `Брокер ${data.requestingBrokerName} (${data.requestingBrokerPhone}) пытался повторно зафиксировать клиента ${data.clientPhone}, но клиент уже на другом брокере / в другой воронке.`,
        ``,
        `ЧТО НУЖНО СДЕЛАТЬ:`,
        `1. Открыть текущий лид клиента — посмотреть на каком этапе он сейчас`,
        `2. Связаться с действующим брокером — узнать актуален ли клиент в работе`,
        `3. Связаться с ${data.requestingBrokerName} (${data.requestingBrokerPhone}) — уточнить ситуацию`,
        `4. Принять решение кому отдать клиента`,
        `5. Отметить решение в комментарии к этой задаче`,
        ``,
        `ПОСЛЕ ВЫПОЛНЕНИЯ:`,
        `— в комментарии: что решили и почему`,
        `— переведите лид (если нужно) на нужного брокера`,
      ].join('\n');
      await this.createTask({
        text: checklistText,
        entityType: 'leads',
