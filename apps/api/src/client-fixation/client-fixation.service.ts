...
export class ClientFixationService {
  ...
  private async handleRule1Or2Alarm(params: {
    ...
    if (amoVerdict.leads && amoVerdict.leads.length > 0) {
      ...
      // 2026-06-30: формат чеклист-задачи для КЦ.
      const taskText = isSameBrokerRefix
        ? [
            `ЗАДАЧА: проверить повторную фиксацию того же брокера.`,
            ``,
            `СИТУАЦИЯ:`,
            `Брокер ${responsibleBroker.fullName} (${responsibleBroker.phone}) ещё раз подал клиента ${data.fullName} (${data.phone}). Лид у этого же брокера уже в работе.`,
            ``,
            `ЧТО НУЖНО СДЕЛАТЬ:`,
            `1. Посмотреть текущий статус существующего лида клиента`,
            `2. Связаться с брокером ${responsibleBroker.fullName} (${responsibleBroker.phone}) — уточнить причину повторной фиксации`,
            `3. Если работа реально продолжается — пометить в комментарии и закрыть задачу`,
            `4. Если есть проблемы с уникальностью — поднять менеджеру`,
            ``,
            `ПОСЛЕ ВЫПОЛНЕНИЯ:`,
            `— в комментарии: причина повторной фиксации и решение`,
          ].join('\n')
        : isRule1
          ? [
              `ЗАДАЧА: разобраться с новым брокером на существующем клиенте.`,
              ``,
              `СИТУАЦИЯ:`,
              `Клиент ${data.fullName} (${data.phone}) уже в КЦ-воронке. Новый брокер ${responsibleBroker.fullName} (${responsibleBroker.phone}) только что прикреплён к лиду контактом.`,
              ``,
              `ЧТО НУЖНО СДЕЛАТЬ:`,
              `1. Открыть лид клиента — посмотреть на каком он этапе`,
              `2. Связаться с НОВЫМ брокером ${responsibleBroker.fullName} (${responsibleBroker.phone}) — уточнить как нашёл клиента`,
              `3. Связаться с действующим брокером (если он уже в карточке) — уточнить статус работы`,
              `4. Принять решение кому отдать клиента или продолжить работу совместно`,
              ``,
              `ПОСЛЕ ВЫПОЛНЕНИЯ:`,
              `— в комментарии: решение по клиенту`,
              `— назначить ответственного брокера`,
            ].join('\n')
          : [
              `ЗАДАЧА: подтвердить уникальность клиента.`,
              ``,
              `СИТУАЦИЯ:`,
              `Клиент ${data.fullName} (${data.phone}) уже в активной стадии (встреча назначена / воронка продаж). Новый брокер ${responsibleBroker.fullName} (${responsibleBroker.phone}) пытается зафиксировать его на себя.`,
              ``,
              `ЧТО НУЖНО СДЕЛАТЬ:`,
              `1. Открыть лид клиента — посмотреть текущего ответственного брокера и стадию`,
              `2. Связаться с действующим брокером — клиент ещё в работе?`,
              `3. Связаться с новым брокером ${responsibleBroker.fullName} (${responsibleBroker.phone}) — объяснить ситуацию`,
              `4. Принять решение: подтвердить уникальность нового или отклонить`,
              ``,
              `ПОСЛЕ ВЫПОЛНЕНИЯ:`,
              `— в комментарии: решение и аргументация`,
              `— переведите Client в админке (uniqueness conflicts) в нужный статус`,
            ].join('\n');
      try {
