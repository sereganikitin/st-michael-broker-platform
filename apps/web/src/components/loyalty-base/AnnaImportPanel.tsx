...
const stepIndex = (step: ImportStep) =>
  stepLabels.findIndex((item) => item.key === step);
...
export function AnnaImportPanel({
  canPublish,
  onPublished,
}: {
  canPublish: boolean;
  ...
  const coverageDropRequiresConfirmation =
    result?.summary.coverageDropRequiresConfirmation === true;
  const hasDryRunSnapshotBinding =
    result?.hasExpectedActiveSnapshotBinding === true;
  const canStage =
    dryRunIsValid &&
  ...
  const chooseFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
        throw new Error(
          "Корнем импортного JSON должен быть объект. Массив backend не принимает.",
        );
      }
      ...
      if (
        ...
        Number(expectedRecords) < 1 ||
        Number(expectedRecords) > 10_000
      ) {
        throw new Error(
          "Поле expectedRecords обязательно и должно быть целым числом от 1 до 10 000.",
        );
      }
      ...
      for (const [field, max, label] of manifestFields) {
        ...
        if (
          ...
          throw new Error(
            `Поле ${field} обязательно: укажите ожидаемое число ${label} от 0 до ${max.toLocaleString("ru-RU")}.`,
          );
        }
      }
      if (
        typeof document.expectedIncludedDealAmount !== "string" ||
        !/^\d{1,16}(?:\.\d{1,2})?$/.test(document.expectedIncludedDealAmount)
      ) {
        throw new Error(
          'Поле expectedIncludedDealAmount обязательно: укажите точную сумму включённых сделок строкой в рублях, например "4722766207.00".',
        );
      }
  ...
  const runDryRun = async () => {
    ...
    try {
      ...
      if (!next.contentHash)
        throw new Error("Backend не вернул contentHash; staging заблокирован.");
      setResult(next);
  ...
  const stage = async () => {
    if (!canPublish) {
      setError(
        "Staging и публикация доступны только администратору. Dry-run завершён без изменения данных.",
      );
      return;
    }
    ...
    try {
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "Не удалось создать staged snapshot",
      );
    } finally {
      setBusy(false);
    }
  ...
  const publish = async () => {
    if (!canPublish) {
      setError("Публикация доступна только администратору.");
      return;
    }
    ...
    if (!result.hasExpectedActiveSnapshotBinding) {
      setError(
        "Backend не вернул expectedActiveSnapshotId для привязки publish. Повторите dry-run и staging.",
      );
      return;
    }
    if (
      !window.confirm(
        "Опубликовать staged snapshot в «Базу Анны Скибицкой»? Данные «Нашей базы» не будут изменены.",
      )
    )
      return;
    setBusy(true);
    ...
    try {
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "Не удалось опубликовать snapshot",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <section className="card space-y-4" aria-labelledby="anna-import-title">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">
            JSON проходит dry-run и staging. Publish доступен только после
      ...
      <ol
        ...
        aria-label="Этапы импорта"
      >
        {stepLabels.map((item, index) => (
          <li
            key={item.key}
            className={`rounded-lg px-3 py-2 text-xs font-medium border ${index <= stepIndex(step) ? "bg-accent/10 text-accent border-accent/20" : "bg-background text-text-muted border-border"}`}
          >
            {item.label}
          </li>
        ))}
      ...
      <label className="block rounded-xl border-2 border-dashed border-border hover:border-accent/50 p-5 cursor-pointer">
        <input
          ...
          onChange={chooseFile}
          disabled={busy || step !== "select"}
        />
        <span className="flex items-center gap-3">
          <FileJson className="w-8 h-8 text-accent" />
          <span>
            <b>{fileName || "Выбрать JSON-файл"}</b>
            <small className="block text-text-muted mt-1">
              Файл читается в память текущей сессии и не сохраняется в
              localStorage.
            </small>
          </span>
        </span>
      ...
      {step === "dry-run" && !dryRunIsValid && (
        <div className="rounded-lg bg-warning/10 text-warning p-3 flex gap-2 text-sm">
          <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
          Dry-run не разрешил публикацию. Исправьте указанные ошибки в исходном
          файле и запустите импорт заново.
        </div>
      )}
      ...
      {step === "dry-run" && dryRunIsValid && !hasDryRunSnapshotBinding && (
        <div className="rounded-lg bg-error/10 text-error p-3 flex gap-2 text-sm">
          <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
          Backend не вернул привязку к активному snapshot. Staging заблокирован:
          повторите dry-run.
        </div>
      )}
        ...
        coverageDropRequiresConfirmation && (
          <label className="flex items-start gap-3 rounded-lg border border-warning/40 bg-warning/10 p-4 text-sm">
            <input
              className="mt-1"
              type="checkbox"
              checked={coverageDropConfirmed}
              onChange={(event) =>
                setCoverageDropConfirmed(event.target.checked)
              }
            />
            <span>
              ...
              <span className="block text-text-muted mt-1">
                Уменьшаются:{" "}
                {result.summary.coverageDrops.length > 0
                  ? result.summary.coverageDrops
                      .map(
                        (drop) =>
                          `${coverageDimensionLabels[drop.dimension] || drop.dimension}: ${drop.current} → ${drop.staged}`,
                      )
                      .join("; ")
                  : `${result.summary.currentPublishedRecords ?? "—"} → ${result.summary.records} записей`}
                . Staging и publish будут заблокированы без подтверждения для
                этого перехода.
              </span>
            </span>
          </label>
      ...
      {result && (
        <div className="rounded-xl border border-border overflow-hidden">
          <div className="px-4 py-3 bg-surface-secondary font-medium">
            Результат этапа {result.status ? `· ${result.status}` : ""}
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-border border-y border-border text-sm">
            {(
              [
                ...
                ...(result.summary.includedActivities === null
                  ? []
                  : [["Активностей в KPI", result.summary.includedActivities]]),
                ...(result.summary.includedFixations === null
                  ? []
                  : [["Фиксаций в KPI", result.summary.includedFixations]]),
                ...(result.summary.includedMeetings === null
                  ? []
                  : [["Встреч в KPI", result.summary.includedMeetings]]),
                ...(result.summary.includedDeals === null
                  ? []
                  : [["Сделок в KPI", result.summary.includedDeals]]),
                ...(result.summary.includedBrokerTours === null
                  ? []
                  : [
                      [
                        "Брокер-туров в KPI",
                        result.summary.includedBrokerTours,
                      ],
                    ]),
                ...(result.summary.includedCalls === null
                  ? []
                  : [["Звонков в KPI", result.summary.includedCalls]]),
                ...(result.summary.includedDealAmount === null
                  ? []
                  : [
                      [
                        "Сумма сделок в KPI",
                        `${result.summary.includedDealAmount} ₽`,
                      ],
                    ]),
                ...(result.summary.excludedActivities === null
                  ? []
                  : [["Исключено из KPI", result.summary.excludedActivities]]),
                ...(result.summary.currentPublishedRecords === null
                  ? []
                  : [
                      [
                        "Сейчас опубликовано",
                        result.summary.currentPublishedRecords,
                      ],
                    ]),
          ...
          {result.summary.unknownActivities !== null &&
            result.summary.unknownActivities > 0 && (
              <div className="border-t border-warning/25 bg-warning/10 text-warning p-3 text-sm flex gap-2">
                <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
                Требуют проверки (не входят в KPI):{" "}
                {result.summary.unknownActivities}
              </div>
            )}
        </div>
      )}
      ...
      {step === "staged" && (
        <label className="flex items-start gap-3 rounded-lg border border-warning/30 bg-warning/5 p-4 text-sm">
          ...
          />
          <span>
            <b>Подтверждаю publish</b>
            <span className="block text-text-muted mt-1">
              Публикуется только staged snapshot для базы Анны. «Наша база» и
              источные записи не перезаписываются.
            </span>
          </span>
        </label>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        {step === "select" && (
          <button
            ...
            onClick={runDryRun}
          >
            {busy && <Loader2 className="w-4 h-4 animate-spin" />}1. Запустить
            dry-run
          </button>
        )}
        {step === "dry-run" && canPublish && (
          <button
            type="button"
            className="btn btn-primary inline-flex items-center gap-2"
            ...
            onClick={stage}
          >
            {busy && <Loader2 className="w-4 h-4 animate-spin" />}2. Создать
            staged snapshot
          </button>
        )}
        {step === "dry-run" && !canPublish && (
          <div className="rounded-lg bg-success/10 px-3 py-2 text-sm text-success">
            Dry-run завершён. Staging и publish доступны только администратору.
          </div>
        )}
        {step === "staged" && canPublish && (
          <button
            type="button"
            className="btn btn-primary inline-flex items-center gap-2"
            ...
            onClick={publish}
          >
            {busy && <Loader2 className="w-4 h-4 animate-spin" />}3.
            Опубликовать
          </button>
        )}
        {step === "published" && (
